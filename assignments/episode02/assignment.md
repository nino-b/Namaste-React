...
- Share packages with other developers. Developers can publish their packages to the <b>npm registry</b>.
  - In such case, your project's <b>package.json</b> file must contain a property - <b>"main"</b>. <b>"main"</b> should take JS file as entry point, instead of defauld HTML file. 
...
- <b>Installation:</b> You use npm to install a package from the npm registry (command: ```npm install package-name```). 
...
<b>Bundler is a tool that combines all modules and files into a single file (or a few files).</b>

- Analyzes code and understands connections between modules or files.
- Uses transpilers to make code compatible for older browsers.
- Minifies and Optimizes code. Removes white space and all unnecessary characters and dependencies (tree shaking), optimizes images.
- HMR (Hot Module Replacement) feature allows developers to see changes in real time without full page reload. 
...
- .parcel-cache is a directory generated by Parcel, that stores cached data from the prject.
- cache includes all files that Parcel has processed.
- By caching data Parcel avoids redoing work that has been done in previous builds, thus reduces build times.
- .parcel-cache should be excluded from version control - it should be in ```.gitignore``` file, because it can get very large and its contents are specific to each developer's environment.
- We should clear .parcel-cache if there were made significant changes to the project's configuration, outdated data is being served, or there is some odd behavior that is not happening with your teammates. To clear cahe, use command: <b>rm -rf .parcel-cache</b>
...
- A package runner tool that comes with Node.js (it is installed automatically).
- Executes Node packages without having to install them globally. 
- Useful for running packages, testing them, or executing commands from packages not currently installed on your system.
...
- <b>dependencies:</b> Lists the packages your project depends on to run. npm automatically installs these packages when someone installs your project.
- <b>devDependencies:</b> Lists the packages needed for development and testing, but not for running the project in production.
...
- A process of optimization that reduces the size of a project's final bundle. 
- Removes code that isn't actually used (referred to as "dead code") from the final output. 
- Beneficial for applications that use large libraries or frameworks, as it ensures that only the code that is actually needed by the application is included in the build.
...
- Hot Module Replacement (HMR) is a bundler's feature.
- Allows for real-time updating of modules in a browser without needing a full refresh. 
- This means that when you make changes to your code, the updates can be applied live as you develop, without losing the current state of the application. 
- HMR enhances the development experience by making it faster and more efficient.
...
- <b>Dev Build (development 'mode'):</b> creates optimized environment for developers to write, test and debug code. It prioritizes smooth development proess over end-user's experience, like: detailed logging, debugging features, HMR, quick compilation. 
- <b>Local Server:</b> Provides a built-in development server that makes it easy to develop and test web applications locally.
- <b>HMR (Hot Module Replacement):</b> allows developers to see updates in the browser without needing a full page refresh, preserving the state of the application during development.
- <b>File Watching Algorithm:</b> detects changes in the project files. Results in quicker rebuilds by only processing files that have changed.
- <b>Caching:</b> caches compiled assets and modules. Once a file is processed, it won't need to be re-processed in future builds unless it changes, leading to significantly faster build times.
- <b>Image Optimization:</b> reduces image file size without losing quality, which can improve load times.
- <b>Minification:</b> removes unnecessary characters from code without changing its functionality.
- <b>Bundling:</b> combines all the modules and assets your application depends on into a smaller number of files, optimizing load times.
- <b>Compression:</b> compresses bundled assets using algorithms like Gzip or Brotli.
- <b>Consistent Hashing:</b> Uses consistent hashing for output filenames, ensuring that file names only change when the content of the files changes, which improves browser caching.
- <b>Code Splitting:</b> Allows you to split your code into separate bundles that can be loaded on demand, improving initial load time by only loading what's necessary upfront.
- <b>Differential Bundling:</b> Supports creating different bundles for modern and older browsers.
- <b>Diagnostic:</b> Provides detailed diagnostic messages and error reports to help identify and fix issues quickly during development.
- <b>Error Handling:</b> Robust error handling mechanisms improve the development experience by clearly indicating issues and potential fixes.
- <b>HTTPS:</b> The development server can serve files over HTTPS, making it easier to test features that require a secure context.
- <b>Tree Shaking:</b> Eliminates unused code from your bundles, ensuring that only the code that is actually used is included in the final build, reducing the size and improving the performance of your application.
- <b>Different dev and production bundles:</b> Automatically differentiates between development and production builds, optimizing the latter for performance by applying additional optimizations like minification and compression.
